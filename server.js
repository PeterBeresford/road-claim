...
<!-- APPROVER BAR -->
<div class="approver-bar" id="approverBar">
...
<div class="tab-content" id="tab-submissions">
  ...
  <div class="drop-zone" id="dropZone" onclick="document.getElementById('fileInput').click()">
    ...
    <h3>Drop driver expense files here</h3>
    <p>Drag and drop or click to import .json files exported from the Driver Expenses Form v1.3</p>
    <input type="file" id="fileInput" multiple accept=".json" onchange="handleFiles(this.files)">
  </div>

  <div class="panel" style="margin-bottom:1.5rem">
    <div class="panel-header"><div class="panel-title">Import from email</div></div>
    <div style="padding:1rem">
      <p style="font-size:0.82rem;color:var(--mid);margin-bottom:0.75rem">Copy everything between <strong>=== DASHBOARD DATA ===</strong> and <strong>=== END DATA ===</strong> from the driver email and paste it here.</p>
      <textarea id="pastebox" style="width:100%;height:80px;font-family:monospace;font-size:0.75rem;border:1px solid var(--rule);padding:0.5rem;border-radius:2px;resize:vertical" placeholder="Paste driver data here..."></textarea>
      <div style="margin-top:0.5rem;display:flex;align-items:center;gap:1rem">
        <button class="btn btn-ink" onclick="importFromPaste()">Import</button>
        <span id="pasteMsg" style="font-size:0.78rem"></span>
      </div>
    </div>
  </div>
...
<script>
  ...
  function loadFromServer() {
    fetch('/submissions').then(function(r){return r.json();}).then(function(data){
      submissions = data;
  ...
  function importFromPaste() {
    ...
    if (!raw) { msg.textContent = 'Nothing pasted.'; msg.style.color = 'red'; return; }
    var start = raw.indexOf('{');
    var end = raw.lastIndexOf('}');
    if (start < 0 || end < 0) { msg.textContent = 'No data found. Copy the block between the === markers.'; msg.style.color = 'red'; return; }
    try {
      var data = JSON.parse(raw.substring(start, end + 1));
      var key = data.driver + '|' + (data.exportedAt || data.submittedAt || '');
      if (!submissions.find(function(s){ return s.driver + '|' + (s.exportedAt||s.submittedAt||'') === key; })) {
        submissions.push(data);
        if (!approvalStatus[key]) approvalStatus[key] = 'pending';
      }
      renderAll(); prefetchRates();
      document.getElementById('pastebox').value = '';
      msg.textContent = 'Imported: ' + (data.driver||'Driver') + ' ✓';
      msg.style.color = 'green';
      setTimeout(function(){ msg.textContent = ''; }, 4000);
    } catch(e) {
      msg.textContent = 'Could not read the data. Check you copied it correctly.';
      msg.style.color = 'red';
    }
  }
  ...
  function renderSubmissions() {
    ...
    submissions.forEach(function(d) {
      ...
      html += '<button class="status-btn" style="color:var(--red);border-color:var(--red)" onclick="removeSubmission(\\'' + key + '\\')">✕ Remove</button>';
      html += '</div>';
  ...
  function removeSubmission(key) {
    ...
    if (sub) {
      fetch('/submissions/'+sub.id,{method:'DELETE'}).then(function(){loadFromServer();});
    }
  }
  ...
  function setStatus(key, status) {
    ...
    fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'}).then(function(){loadFromServer();});
    approvalStatus[key] = status;
    if (status === 'approved') {
      var sub = submissions.find(function(s) { return s.driver + '|' + (s.receivedAt||s.submittedAt||'') === key; });
      if (sub && sub.email) {
        var total = driverTotal(sub);
        var subject = encodeURIComponent('Expenses Approved: ' + (sub.tour || 'Tour') + ' — ' + (sub.driver || 'Driver'));
        var body = encodeURIComponent(
          'Hi ' + (sub.driver || 'there') + ',\\n\\n' +
          'Your expenses for ' + (sub.tour || 'your recent tour') + ' have been approved.\\n\\n' +
          'Tour:       ' + (sub.tour || '—') + '\\n' +
          'Dates:      ' + (sub.dateOut || '—') + ' to ' + (sub.dateReturn || '—') + '\\n' +
          'Total:      £' + total.toFixed(2) + '\\n' +
          'Approved:   ' + new Date().toLocaleDateString('en-GB') + '\\n\\n' +
          'Payment will be processed in the normal way.\\n\\n' +
          'Transam Accounts'
        );
